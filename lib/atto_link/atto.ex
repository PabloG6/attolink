defmodule AttoLink.Atto do
  ...
  def cache_preview(
    ...
    with {:ok, _sum} <- check_html_throttle(user),
         ...
         {:ok, %Preview{}} = result <-
           Preview.changeset(
             %Preview{},
             Map.from_struct(attrs) |> Enum.into(%{path: path, byte_size: byte_size, user_id: id})
           )
           |> Repo.insert() do
      result
    else
      ...
      {:deny, :exceeded_file_store_limit, _limit} = err ->
        err
  ...
  defp save_html_page(email, %LinkPreview.Page{original_url: original_url, title: title}) do
    {:ok, %Tesla.Env{body: body}} =
      case Tesla.get(original_url, headers: [{"User-Agent", @user_agent}, {"accept", "/"}]) do
    ...
    if File.exists?(path) do
      ...
      joined_path =
        [path, "#{title |> to_snake()}_#{current_time}.html"]
        |> Path.join()
        |> Path.absname()
