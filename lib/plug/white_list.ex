defmodule AttoLink.Plug.WhiteList do
  ...
  defp check_ip(ip, %Plug.Conn{} = conn) do
    with {:ok, %AttoLink.Accounts.User{id: id} = user} <- current_user(conn),
         {:ok, %AttoLink.Security.Permissions{enable_whitelist: :restricted}} <-
           AttoLink.Security.get_permissions_by!(user_id: id),
      ...
      {:error, :unverified_ip} ->
        ...
        |> resp(
          401,
          Poison.encode!(%{
            message: "This ip address has not been white listed",
            response_code: :unregistered_ip
          })
        )
        |> send_resp()
        |> halt()
      ...
      {:ok, %AttoLink.Security.Permissions{enable_whitelist: :none}} ->
        ...
        |> resp(
          401,
          Poison.encode!(%{
            message: "This account has restricted all IPs",
            response_code: :restricted
          })
        )
        |> send_resp()
        |> halt()
